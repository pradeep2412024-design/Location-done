...
function generateWeatherRecommendations(phase, weatherRisk, weatherData) {
  ...
  // Temperature-based recommendations (optimized)
  const avgTemp = weatherData.length > 0 ? 
    weatherData.reduce((sum, day) => sum + (day.tempValue || parseInt(day.temp) || 0), 0) / weatherData.length : 25
...
function calculateOdishaWeatherRisk(weatherData, month, crop, location) {
  // Optimized calculations with safety checks
  const avgTemp = weatherData.length > 0 ? 
    weatherData.reduce((sum, day) => sum + (day.tempValue || parseInt(day.temp) || 0), 0) / weatherData.length : 25
  const rainyDays = weatherData.length > 0 ? 
    weatherData.filter(day => day.condition && day.condition.includes("rain")).length : 0
  const avgHumidity = weatherData.length > 0 ? 
    weatherData.reduce((sum, day) => sum + (day.humidityValue || parseInt(day.humidity) || 50), 0) / weatherData.length : 60
