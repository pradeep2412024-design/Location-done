#!/usr/bin/env python3
"""
Simple ML Console Test - Shows predictions in console
...
def test_yield_prediction():
    ...
    try:
        ...
        if result and 'predictions' in result:
            ...
            print_result("Predicted Yield", f"{yield_val:.2f} tons/hectare")
            
            if confidence:
                lower = confidence.get('lower', 0)
                upper = confidence.get('upper', 0)
                uncertainty = confidence.get('uncertainty', 0)
                print_result("Confidence Range", f"{lower:.2f} - {upper:.2f} tons/hectare")
                print_result("Uncertainty", f"+/-{uncertainty:.2f} tons/hectare")
        else:
            print("ERROR: Yield prediction failed - no predictions returned")
            
    except Exception as e:
        print(f"ERROR: Yield prediction error: {e}")
...
def test_crop_recommendation():
    ...
    try:
        ...
        if result and 'recommendations' in result:
            ...
            print("ERROR: Crop recommendation failed - no recommendations returned")
            
    except Exception as e:
        print(f"ERROR: Crop recommendation error: {e}")
    ...
    try:
        ...
        if result:
            ...
            if crop_recs:
                ...
                print_result("Top Recommended Crop", f"{crop_name} ({score:.1%} - {confidence})")
        else:
            print("ERROR: Comprehensive analysis failed - no result returned")
